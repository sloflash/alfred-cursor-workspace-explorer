...
# Try to find python3 in PATH first
log_debug "Searching for python3 in PATH..."
if command -v python3 &> /dev/null; then
    PYTHON3_PATH=$(command -v python3)
    log_debug "Found python3 in PATH at: $PYTHON3_PATH"
    PYPATHS+=("$(dirname "$PYTHON3_PATH")")
else
...
[ ! -d "$WF_DATA_DIR" ] && mkdir -p "$WF_DATA_DIR"

SCRPATH="$0"
SCRIPT_DIR="$(dirname "$SCRPATH")"

#Cache file for python binary - Allows for faster execution
...
make_alias() {
  ...
  chmod +x "$PYALIAS"
  log_msg "Python3 was found at $PYBIN" "Version: $PYVER, Proceed typing query or re-run workflow"
}
...
setup_python_alias() {
    ...
    if [[ "$p" == *"*"* ]]; then
      ...
      for expanded_path in $p; do
        if [ -d "$expanded_path" ]; then
          log_debug "Checking expanded path: $expanded_path"
          if [ -f "$expanded_path/python3" ] && verify_not_stub "$expanded_path/python3"; then
            make_alias "$expanded_path/python3"
            return 0
          fi
        fi
      done
  ...
  # If we get here, we didn't find Python
  log_debug "No valid Python installation found in any of the searched paths"
  log_msg "Error: no valid python3 version found" "Please ensure Python 3 is installed and accessible. Searched paths: ${PYPATHS[*]}"
  exit 255
}
